...
if game.ReplicatedStorage.Network:WaitForChild("Instancing_FireCustomFromServer") then 
    ...
    local FishingMastery = 0
    game:GetService("ReplicatedStorage").Network["New Stats"].OnClientEvent:Connect(function(a,b)
        if tostring(b) ~= game.Players.LocalPlayer.Name then return end
        for k,v in a do 
            if v[1] == "Mastery" then 
                --for k,v in v[2] do warn(k,v) end
                FishingMastery = v[2].Fishing or 0
            end
        end
    end)
    local CurrentDeepPool = game:GetService("ReplicatedStorage").Network.Instancing_InvokeCustomFromClient:InvokeServer("AdvancedFishing","GetDeepPool")
    game.ReplicatedStorage.Network.Instancing_FireCustomFromServer.OnClientEvent:Connect(function(...) 
        ...
        if Zone == "AdvancedFishing" then 
            if Cmd == "DeepPoolUpdated" then 
                CurrentDeepPool = plr
            end
            local pos = pos
            if FishingMastery > 16100 and CurrentDeepPool then 
                pos = CurrentDeepPool
            end
